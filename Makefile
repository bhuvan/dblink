# This Makefile is for the Dblink extension to perl.
#
...
#     ABSTRACT_FROM => q[Dblink.pm]
#     AUTHOR => q[A.Bhuvaneswaran <bhuvan@symonds.net>]
#     DEFINE => q[]
#     INC => q[]
#     LIBS => [q[]]
#     NAME => q[Dblink]
#     PREREQ_PM => {  }
#     VERSION_FROM => q[Dblink.pm]
...
TO_INST_PM = Dblink.pm

PM_TO_BLIB = Dblink.pm \
	blib/lib/Dblink.pm
...
# Dblink might depend on some other libraries:
...
clean ::
	-rm -rf Dblink.c ./blib $(MAKE_APERL_FILE) $(INST_ARCHAUTODIR)/extralibs.all perlmain.c tmon.out mon.out so_locations pm_to_blib *$(OBJ_EXT) *$(LIB_EXT) perl.exe perl perl$(EXE_EXT) $(BOOTSTRAP) $(BASEEXT).bso $(BASEEXT).def lib$(BASEEXT).def $(BASEEXT).exp $(BASEEXT).x core core.*perl.*.? *perl.core
...
ppd:
	@$(PERL) -e "print qq{<SOFTPKG NAME=\"$(DISTNAME)\" VERSION=\"1,0,0,0\">\n\t<TITLE>$(DISTNAME)</TITLE>\n\t<ABSTRACT>Perl Wrapper for postgresql dblink contrib module</ABSTRACT>\n\t<AUTHOR>A.Bhuvaneswaran &lt;bhuvan\@symonds.net&gt;</AUTHOR>\n}" > $(DISTNAME).ppd
...
pm_to_blib: $(TO_INST_PM)
	...
	-e "pm_to_blib({qw{Dblink.pm blib/lib/Dblink.pm}},'$(INST_LIB)/auto','$(PM_FILTER)')"
	@$(TOUCH) $@
